...
func visit(path string, fileInfo os.FileInfo, err error) error {
	...
	for lineError == nil && !isPrefix {
		...
		if strings.Contains(octopressLineAsString, "categories:") {
			...
			matches = regexp.MustCompile(`^title:\s(.*)`).FindStringSubmatch(octopressLineAsString)
			hugoFileWriter.WriteString("title = \"")
			originalTitle := matches[1]
			if len(originalTitle) > 0 && originalTitle[0] == '"' {
				originalTitle = originalTitle[1:]
			}
			if len(originalTitle) > 0 && originalTitle[len(originalTitle)-1] == '"' {
				originalTitle = originalTitle[:len(originalTitle)-1]
			}
			hugoFileWriter.WriteString(originalTitle)
			hugoFileWriter.WriteString("\"\n")
